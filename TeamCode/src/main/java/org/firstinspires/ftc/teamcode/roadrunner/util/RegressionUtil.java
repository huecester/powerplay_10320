...
public class RegressionUtil {
	 ...
	 * @param file            log file
	 */
	public static RampResult fitRampData(List<Double> timeSamples, List<Double> positionSamples, List<Double> powerSamples, boolean fitStatic, @Nullable File file) {
		if (file != null) {
	 ...
	 * @param file            log file
	 */
	public static AccelResult fitAccelData(List<Double> timeSamples, List<Double> positionSamples, List<Double> powerSamples, RampResult rampResult, @Nullable File file) {
		if (file != null) {
			try (PrintWriter pw = new PrintWriter(file)) {
				pw.println("time,position,power");
				for (int i = 0; i < timeSamples.size(); i++) {
		...
		for (int i = 0; i < timeSamples.size(); i++) {
			...
			double powerFromVel = Kinematics.calculateMotorFeedforward(vel, 0.0, rampResult.kV, 0.0, rampResult.kStatic);
